...
import { Search, Video, Phone, PanelRightOpen } from "lucide-react";
...
const ChatHeader: React.FC<ChatHeaderProps> = ({
  ...
  const { user } = useAuth();
  const { conversations, activeConvId, userProfiles, loadUserProfile } =
    useChatStore();
    ...
    <>
      <header className="h-16 flex items-center justify-between px-6 bg-white/90 dark:bg-surface-container/90 backdrop-blur-xl border-b border-outline-variant/15 dark:border-outline-variant/30 z-20 shrink-0">
        ...
        <div className="flex items-center gap-1.5">
          <button className="w-10 h-10 flex items-center justify-center hover:bg-surface-container rounded-full transition-all text-on-surface-variant hover:text-primary">
            <Search size={20} />
          </button>
          <button
          ...
          {activeChat.type === "group" && (
            <div className="ml-3 hidden sm:flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 text-[12px] font-bold text-on-surface-variant">
