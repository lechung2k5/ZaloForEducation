    ...
    >
      ...
      >
        ...
        <div className="relative group/bubble">
          ...
          {!isRecalled && (
            <div
              className={`absolute top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center gap-1.5 z-[100] ${isMe ? "-left-32 flex-row-reverse animate-in slide-in-from-right-4" : "-right-32 animate-in slide-in-from-left-4"}`}
            >
          ...
          {/* Actual Bubble */}
          <div
            className={`p-3 rounded-2xl shadow-sm border ${bubbleClass} ${isMe ? "border-primary/20" : "border-outline-variant/20"} relative transition-all duration-500 ${isHighlighted ? "animate-pulse-highlight ring-4 ring-amber-400/50 shadow-2xl scale-[1.02]" : ""}`}
          >
            {message.replyTo && (
          ...
          {message.reactions &&
            ...
            !isRecalled && (
              <div
                className={`absolute -bottom-3 flex flex-wrap gap-1 ${isMe ? "right-0" : "left-0"} z-[5]`}
              >
