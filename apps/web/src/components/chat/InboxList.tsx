...
const InboxList: React.FC = () => {
  ...
  const filteredConversations = conversations.filter((conv: any) => {
    ...
    if (tagFilter === "none") return !conv.tagId;
    return conv.tagId === tagFilter;
  });
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1 group">
            ...
            <input
              ...
              className="w-full bg-surface-container-highest border-none rounded-2xl py-2 pl-8.5 pr-4 text-[13px] outline-none text-on-surface placeholder:text-outline focus:ring-2 focus:ring-primary/20 transition-all cursor-text"
      ...
      <div className="flex-1 overflow-y-auto hide-scrollbar p-2 space-y-1">
        {filteredConversations.length === 0 ? (
          ...
          filteredConversations.map((chat) => {
            ...
            const partnerEmail =
              chat.type === "direct"
                ? Array.isArray(chat.members)
                  ? chat.members.find((m) => m !== user?.email)
                  : undefined
                : undefined;
            ...
            const isOnline = partnerEmail
              ? userProfiles[partnerEmail]?.status === "online"
              : false;
