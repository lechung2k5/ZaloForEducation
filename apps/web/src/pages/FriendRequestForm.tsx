...
const FriendRequestForm: React.FC = () => {
  ...
  const renderActionButton = () => {
    ...
    if (searchResult.isSelf) {
      return <span className="text-sm text-slate-500">Đây là tài khoản của bạn</span>;
    }
    ...
    if (friendship.status === 'pending' && friendship.receiverEmail === myEmail) {
      return (
        <button
    ...
    return <span className="text-amber-600 font-medium">Đã gửi lời mời, chờ phản hồi</span>;
  };
    ...
    <div className="min-h-screen bg-slate-50 p-6">
      <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-bold text-slate-800">Kết bạn theo Gmail</h1>
        ...
        {searchResult?.found && (
          <div className="border border-slate-200 rounded-lg p-4 flex items-center justify-between">
            <div>
              <p className="font-semibold text-slate-800">{searchResult.user.fullName}</p>
              <p className="text-sm text-slate-600">{searchResult.user.email}</p>
            </div>
            {renderActionButton()}
          </div>
        )}
        ...
        <div>
          <h2 className="font-semibold text-slate-800 mb-3">Lời mời kết bạn nhận được</h2>
          <div className="space-y-2">
            ...
            {incomingRequests.map((request) => (
              ...
              >
                <span className="text-slate-700">{request.sender_id}</span>
                <button
        ...
        <div>
          <h2 className="font-semibold text-slate-800 mb-3">Bạn bè đã kết nối</h2>
          <div className="space-y-2">
            ...
            {acceptedFriends.map((friend) => {
              const otherEmail =
                friend.sender_id === myEmail ? friend.receiver_id : friend.sender_id;
              return (
                <div key={otherEmail} className="border border-slate-200 rounded-lg p-3">
