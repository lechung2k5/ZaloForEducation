...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const setupSocket = useCallback(async (email: string) => {
    ...
    newSocket.on("force_logout", (data) => {
      ...
      if (shouldLogout) {
        console.log(
          "✅ [SOCKET] Target device matched. Initiating local logout...",
        );
        alert(
          "Phiên đăng nhập của bạn đã hết hạn hoặc bị đăng xuất từ thiết bị khác.",
        );
        logoutLocal();
      } else {
        console.log(
          "[SOCKET] force_logout ignored — not targeting this device",
          {
    ...
    newSocket.on("friend_request_received", (data) => {
      console.log("📣 [SOCKET] friend_request_received EVENT RECEIVED:", data);

      if (typeof window !== "undefined") {
        window.dispatchEvent(
          new CustomEvent("friend-request-received", { detail: data }),
        );
      }
      ...
      Swal.fire({
        ...
        position: "top-end",
        icon: "info",
        title: "Có lời mời kết bạn mới",
        text:
          data?.senderProfile?.fullName ||
          data?.senderProfile?.fullname ||
          data?.senderEmail ||
    ...
    newSocket.on("friendship_updated", (data) => {
      console.log("🔄 [SOCKET] friendship_updated EVENT RECEIVED:", data);
    ...
    newSocket.on("notification:new", (data) => {
      ...
      Swal.fire({
        ...
        text: data?.body || "Bạn vừa nhận một thông báo mới.",
        showConfirmButton: false,
        timer: 4000,
        timerProgressBar: true,
      });
