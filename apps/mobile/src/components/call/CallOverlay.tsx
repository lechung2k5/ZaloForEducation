...
const CallOverlay = () => {
  ...
  const localPanResponder = useRef(
    PanResponder.create({
      onStartShouldSetPanResponder: () =>
        !(useCallStore.getState() as any).isMinimized,
  ...
  const resetAudioMode = useCallback(async () => {
    try {
      ...
      console.log(
        "[CallOverlay] 🛡️ Audio mode reset to system default (released expo-av focus)",
      );
    } catch (e) {
  ...
  */
  ...
  const handleAccept = async () => {
    ...
    if (!activeCallId || useCallStore.getState().callState !== "RINGING")
      return;
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(res.message || "Không thể tham gia cuộc gọi");
  ...
  const handleHangup = async () => {
    ...
    if (start && now - start < 5000) {
      console.warn(
        "[Mobile-Hangup] 🛡️ BLOCKED — call just started",
        now - start,
        "ms ago",
      );
      return;
    ...
    try {
      const durationSec =
        currentStore.callState === "CONNECTED" && start
  ...
  const displayName =
    peer?.fullName || peer?.fullname || peer?.email || "Người dùng UniChat";
      ...
      if (SocketService.socket && conversationId && activeCallId) {
        ...
        setTimeout(() => {
          ...
          if (
            ...
            console.log(
              "[Mobile-Chime] Upgrade request timed out on sender side",
            );
    ...
    <View style={[styles.videoStage, { backgroundColor: "black" }]}>
      ...
      <Animated.View
        style={[
          ...
          { opacity: placeholderOpacity, zIndex: 1 },
        ]}
        pointerEvents="none"
      >
      ...
      {remoteTile && isRemoteCameraOn && (
        <View
          style={[
            ...
            { zIndex: 2, backgroundColor: "transparent" },
          ]}
          pointerEvents="none"
        >
          <RNChimeVideoView
      ...
      >
        <Text style={[styles.actionIcon, { fontSize: 22 }]}>
          close_fullscreen
        </Text>
      </TouchableOpacity>
      ...
      >
        <View style={styles.mediaControls}>
          <TouchableOpacity
            style={[styles.mediaButton, !isMicOn && styles.mediaButtonActive]}
            onPress={() => setMicOn(!isMicOn)}
          >
            <Text
              style={[styles.mediaIcon, !isMicOn && styles.mediaIconActive]}
            >
              {isMicOn ? "mic" : "mic_off"}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[
              styles.mediaButton,
              isSpeakerOn && styles.mediaButtonActive,
            ]}
            onPress={() => {
          ...
          >
            <Text
              style={[styles.mediaIcon, isSpeakerOn && styles.mediaIconActive]}
            >
          ...
          <TouchableOpacity
            style={[
              styles.mediaButton,
              !isCameraOn && styles.mediaButtonActive,
            ]}
            onPress={handleToggleCamera}
          >
            <Text
              style={[styles.mediaIcon, !isCameraOn && styles.mediaIconActive]}
            >
  ...
  const renderAudioCall = () => (
    <View style={styles.content}>
      ...
      >
        <Text style={[styles.actionIcon, { fontSize: 22, color: "#fff" }]}>
          close_fullscreen
        </Text>
      </TouchableOpacity>
        ...
        <View style={styles.mediaControls}>
          ...
          >
            <Text style={[styles.mediaIcon]}>
              {upgradeRequestPending ? "hourglass_empty" : "videocam"}
            </Text>
          </TouchableOpacity>
        </View>
        <TouchableOpacity
        ...
        <SafeAreaView style={{ flex: 1 }}>
          ...
          {incomingUpgradeRequest && callState === "CONNECTED" && (
            ...
            >
              ...
              >
                {displayName} muốn chuyển sang cuộc gọi Video
              </Text>
