  ...
  return (
    ...
    >
      {!isMe && (
        <Image
          source={{
            uri: userProfile?.avatarUrl || getDisplayAvatar(message.senderId),
          }}
          style={styles.avatar}
        />
      )}
            ...
            (Array.isArray(message.media) || Array.isArray(message.files)) && (
              <View style={styles.mediaContainer}>
                ...
                <View style={styles.imageGrid}>
                  {(Array.isArray(message.media) ? message.media : []).map(
                    (item, index) => {
                            ...
                            <View style={styles.mediaBadgeRow}>
                              {isSticker && (
                                <View style={styles.stkBadge}>
                                  <Text style={styles.badgeText}>STK</Text>
                                </View>
                              )}
                              {isHD && (
                ...
                <View style={styles.fileList}>
                  {(Array.isArray(message.files) ? message.files : []).map(
                    (item, index) => {
                      const file = normalizeAttachment(item);
                      return (
                        <TouchableOpacity
          ...
          >
            {reactionSummary.map(([emoji, users]) => (
              <TouchableOpacity
                ...
                onPress={() => onReaction && onReaction(message, emoji)}
              >
                <Image
                  source={{ uri: FLUENT_EMOJI_MAP[emoji] || "" }}
                  style={styles.reactionEmojiIcon}
                />
