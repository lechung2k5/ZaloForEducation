...
> = (set, get) => ({
  ...
  clearHistory: async (convId: string) => {
    try {
      await chatPost(
        `/conversations/${encodeURIComponent(convId)}/clear-history`,
        {},
      );
    ...
    try {
      ...
      const rawMessages = Array.isArray(payload?.messages)
        ...
        : Array.isArray(payload)
          ? payload
          : [];
      const formattedMessages = sortMessages(
  ...
  fetchMoreMessages: async (
    ...
    try {
      ...
      const payload = res?.data || {};
      const rawMore = Array.isArray(payload?.messages)
        ? payload.messages
        : Array.isArray(payload)
          ? payload
          : [];
  ...
  fetchMessagesContext: async (convId, targetId, token) => {
    ...
    try {
      const res = await chatGet(
        `/conversations/${encodeURIComponent(convId)}/messages-context/${encodeURIComponent(targetId)}`,
      );
      if (token !== get().fetchToken) return;
      if (res.ok && res.data) {
  ...
  setMessages: (updater, nextCursor) =>
    set((state) => {
      ...
      const safeMessages = sortMessages(
        dedupped.map(normalizeMessage).filter((m): m is any => !!m),
      );
  ...
  addMessage: (message) =>
    set((state) => {
      ...
      if (convIndex !== -1) {
        ...
        const isFromOthers =
          safeMessage.senderId && myEmail && safeMessage.senderId !== myEmail;
      ...
      const isActive =
        incomingConvId &&
        state.activeConvId &&
        incomingConvId.toLowerCase() === state.activeConvId.toLowerCase();
  ...
  sendMessageOptimistic: (
    ...
    const optimisticMsg = {
      ...
      content,
      type:
        msgType === "text" && (media.length > 0 || files.length > 0)
          ? "media"
          : msgType,
      status: "sending",
      createdAt: timestamp,
    ...
    set((state) => {
      ...
      if (convIndex !== -1) {
        ...
        target.lastMessageContent =
          typeof content === "string"
            ? content
            : (content as any)?.text || "[Tin nhắn]";
  ...
  deleteMessageOptimistic: async (convId, messageId) => {
    try {
      ...
      await chatPost(
        `/conversations/${encodeURIComponent(convId)}/messages/${encodeURIComponent(messageId)}/delete`,
        {},
      );
    } catch (err) {}
  },
    ...
    try {
      ...
      set((state) => {
        const nextMessages = state.messages.map((m) => {
          ...
          const poll = m.payload?.poll || m.poll;
          if (!poll) return m;

          const userEmail = String(get().currentUserEmail || "")
            .trim()
            .toLowerCase();
      ...
      // API call
      const res = await chatPost(
        `/conversations/${encodeURIComponent(convId)}/messages/${encodeURIComponent(messageId)}/poll/vote`,
        { optionIndex },
      );
