...
import { formatReminderDateTime } from "../../utils/reminderNotifications";
...
export default function ChatScreen({
  ...
  const {
    ...
    targetEmail,
    startCall,
    targetMessageId: paramTargetMessageId,
  } = params || {};
  ...
  const processedMessages = useMemo(() => {
    ...
    for (let i = 0; i < sorted.length; i++) {
      ...
      // Kiểm tra Grouping
      const isMe =
        normalizeEmail(current.senderId) === normalizeEmail(currentUserEmail);
      const prevIsMe =
        prev &&
        normalizeEmail(prev.senderId) === normalizeEmail(current.senderId);
      const nextIsMe =
        next &&
        normalizeEmail(next.senderId) === normalizeEmail(current.senderId);
      ...
      if (prevIsMe && diffPrev < gapLimit && nextIsMe && diffNext < gapLimit)
      ...
      results.push({
        ...
        groupPosition,
        showAvatar:
          !isMe && (groupPosition === "last" || groupPosition === "single"),
      });
  ...
  const performScrollToMessage = useCallback(() => {
    ...
    const index = processedMessages.findIndex(
      (m) => m.id === targetMessageId || m.SK === targetMessageId,
    );
    if (index !== -1) {
      console.log(
        "[ChatScreen] Force scrolling to index",
        index,
        "for message",
        targetMessageId,
      );
    ...
    const initChat = async () => {
      ...
      if (activeId) {
          ...
          (async () => {
            ...
            try {
              const res = await chatGet(
                `/conversations/${encodeURIComponent(activeId)}/metadata`,
              );
        ...
        if (SocketService.socket)
          SocketService.socket.emit("join_room", { convId: activeId });
        useChatStore.getState().markReadLocal(activeId);
        markNotificationsRead(activeId);
        chatPatch(
          `/conversations/${encodeURIComponent(activeId)}/read`,
          {},
        ).catch(() => {});
      }
    };
  ...
  useEffect(() => {
    if (selectedChat?.type === "direct" && selectedChat?.members) {
      const partnerEmail = selectedChat.members.find(
  ...
  useEffect(() => {
      ...
      if (
        ...
        useChatStore
          ...
          .setMessages((prev: any[]) =>
            prev.filter((m) => m.id !== (data.messageId || data.id)),
          );
      }
    };
    ...
    const handleGroupUpdate = (data: any) => {
      const convId = data.convId || data.id;
      if (convId !== conversationId) return;

      // Merge updates for conversation list
      setConversations((prev: any[]) =>
        prev.map((c) => {
          if (c.id !== convId) return c;
          const next: any = { ...c };
          // Prefer explicit members payload if provided
          if (data.members) next.members = data.members;
          if (data.addedMembers)
            next.members =
              data.members ||
              Array.from(new Set([...(c.members || []), ...data.addedMembers]));
          if (data.removedMember)
            next.members =
              data.members ||
              (c.members || []).filter((m: string) => m !== data.removedMember);
          if (data.infoUpdated) {
          ...
          if (data.roleUpdated) {
            if (data.roleUpdated.role === "owner") {
              ...
              next.admin = data.roleUpdated.email;
            }
      ...
      // Update localConversation if open
      if (
        data.members ||
        data.updates ||
        data.infoUpdated ||
        ...
        setLocalConversation((prev: any) =>
          prev
            ? {
                ...prev,
                ...(data.updates || {}),
                ...(data.members ? { members: data.members } : {}),
                ...(data.infoUpdated?.name
                  ? { name: data.infoUpdated.name }
                  : {}),
                ...(data.infoUpdated?.avatar
                  ? { avatar: data.infoUpdated.avatar }
                  : {}),
                ...(data.roleUpdated && data.roleUpdated.role === "owner"
                  ? { owner: data.roleUpdated.email }
                  : {}),
              }
            : prev,
        );
      ...
      // Notify user immediately about membership changes
      try {
        if (
          data.addedMembers &&
          ...
          const actorName = getDisplayName(data.actor);
          const names = data.addedMembers
            .map((e: string) => getDisplayName(e))
            .join(", ");
          Alert.alert(
            "Cập nhật nhóm",
        ...
        if (data.removedMember) {
          const removed = String(data.removedMember).trim().toLowerCase();
          const me = normalizeEmail(user?.email);
          const actorName = getDisplayName(data.actor);

          if (removed === me) {
            // Current user was removed — notify and navigate away
            Alert.alert("Thông báo", "Bạn đã bị xóa khỏi nhóm");
            if (goBack) goBack();
            else setActiveConversation(null);
            return;
          } else {
            const targetName = getDisplayName(data.removedMember);
            Alert.alert(
              "Cập nhật nhóm",
    ...
    const handleConversationUpdated = (data: any) => {
      ...
      if (convId === conversationId) {
        setConversations((prev: any[]) =>
          prev.map((c) => (c.id === convId ? { ...c, ...data.updates } : c)),
        );
        if (data.updates) {
          setLocalConversation((prev: any) =>
  ...
  const handleChatSend = async (content: string, attachments: any[] = []) => {
    ...
    // Detect special poll attachment
    const pollAttachment = (attachments || []).find(
      (a: any) =>
        a &&
        (a.mimeType === "application/poll" ||
          a.name === "poll.json" ||
          (a.data && a.data.topic)),
    );
    const reminderAttachment = (attachments || []).find(
      (a: any) =>
        a &&
        (a.mimeType === "application/reminder" ||
          a.name === "reminder.json" ||
          (a.data && a.data.title && a.data.time)),
    );
    const isPoll = !!pollAttachment;
    const isReminder = !!reminderAttachment;
    const msgType = isPoll ? "poll" : isReminder ? "system" : "text";

    // 1. Send optimistic message immediately
    const tempId = sendMessageOptimistic(
      chatId,
      currentUserEmail || "",
      content,
      msgType,
      {
        ...
        replyTo: replyToData,
        skipApi: true,
        payload: isPoll ? { poll: pollAttachment.data } : undefined,
      },
    );
    ...
    (async () => {
      try {
        ...
        if (isPoll) {
          // Directly post poll payload to backend
          const pollPayload = {
            content:
              content || `[Bình chọn: ${pollAttachment.data?.topic || ""}]`,
            type: "poll",
            payload: { poll: pollAttachment.data },
            replyTo: replyToData,
          };
        ...
        if (isReminder) {
          const reminderData = reminderAttachment.data || {};
          const reminderSchedule = formatReminderDateTime(
            reminderData.date,
            reminderData.time,
          );
          const reminderContent = reminderSchedule
            ? `[Nhắc hẹn] ${reminderData.title || content || ""} • ${reminderSchedule}`
            : `[Nhắc hẹn] ${reminderData.title || content || ""}`;
        ...
        const uploaded = await Promise.all(
          attachments.map(async (item) => {
            ...
            return res.ok
              ? { ...item, dataUrl: res.data.fileUrl || res.data.url }
              : item;
          }),
        );
        ...
        const isVideo = (a: any) => {
          ...
          return (
            ...
            /\.(mp4|mov|avi|wmv|webm|mkv|3gp|flv|m4v)(\?.*)?$/.test(name)
          );
        };
        const isImage = (a: any) => {
          const mime = String(a.mimeType || "").toLowerCase();
        ...
        const media = uploaded
          .filter((a: any) => isImage(a) || isVideo(a))
          .map((a: any) => ({
            url: a.dataUrl,
            dataUrl: a.dataUrl,
            name: a.name,
        ...
        if (res.ok && res.data) {
          ...
          console.error(
            `[ChatScreen] Background send failed for ${tempId}:`,
        ...
        console.error(
          `[ChatScreen] Background send exception for ${tempId}:`,
          err,
        );
        updateMessage(tempId, { status: "error" });
      }
  ...
  const handleStartCall = async (type: "audio" | "video") => {
    if (!selectedChat || selectedChat.type !== "direct")
      return Alert.alert("Thất bại", "Chỉ hỗ trợ gọi 1:1");
    const partnerEmail =
      ...
      (Array.isArray(selectedChat.members)
        ...
        : undefined);
    if (!partnerEmail)
      return Alert.alert("Lỗi", "Không tìm thấy thông tin đối phương");
    const activeCallId = uuidv4();
    startOutgoingCall(
      {
        email: partnerEmail,
        fullName: getDisplayName(partnerEmail),
        avatarUrl: getDisplayAvatar(partnerEmail),
      },
      type,
      selectedChat.id,
      activeCallId,
    );
    const res = await apiPost("/call/create", {
  ...
  const toggleReaction = async (message: any, emoji: string) => {
    ...
    patchMessageOptimistic(selectedChat.id, message.id, {
      action: "react",
  ...
  const isOnline =
    selectedChat?.type === "direct" && partnerProfile?.status === "online";
  const typingText =
    typingUsers.size > 0
      ? `${getDisplayName([...typingUsers][0])} đang gõ...`
      : selectedChat?.type === "direct"
        ? partnerProfile?.statusMessage ||
          partnerProfile?.currentStatus ||
          (isOnline ? "Đang hoạt động" : "Vừa mới truy cập")
      ...
      <View style={styles.container}>
        ...
        >
          <ChatHeader
            ...
            displayAvatar={
              selectedChat.type === "direct"
                ? getDisplayAvatar(partner)
                : selectedChat.avatar || getDisplayAvatar()
            }
            isOnline={
              selectedChat.type === "direct" &&
              userProfiles[partner]?.status === "online"
            }
            typingText={typingText}
            onStartCall={handleStartCall}
            onOpenDetails={() => {
              if (onNavigate)
                onNavigate("ChatDetails", { conversationId: selectedChat.id });
            }}
          />

          <PinBanner
            activePinnedMessages={activePinnedMessages}
            isPinsExpanded={isPinsExpanded}
            setIsPinsExpanded={setIsPinsExpanded}
            onJumpToMessage={(id) => setActiveConversation(selectedChat.id, id)}
            onUnpin={(id) =>
              chatPatch(
                `/conversations/${encodeURIComponent(selectedChat.id)}/messages/${encodeURIComponent(id)}`,
                { action: "unpin" },
              )
            }
          />
          ...
          <FlatList
            ...
            renderItem={({ item: m }) => {
              ...
              if (
                m.type === "call" ||
                m.type === "SYSTEM_CALL" ||
                (m.type === "system" && m.content?.includes("Cuộc gọi"))
              )
                return (
              ...
              if (m.type === "system")
                return (
                  <SystemNotificationItem
                    message={m}
                    onJump={(id) => setActiveConversation(selectedChat.id, id)}
                  />
                );
              ...
              const isSeen =
                ...
                m.id === selectedChat.lastReadMessageId;

              return (
                <MessageBubble
                  message={m}
                  isMe={
                    normalizeEmail(m.senderId) ===
                  ...
                  onReply={handleReply}
                  onReplyPress={(id) =>
                    setActiveConversation(selectedChat.id, id)
                  }
                  onSystemMessagePress={(id) =>
                    setActiveConversation(selectedChat.id, id)
                  }
                  isHighlighted={
                    !!targetMessageId &&
                    (m.id === targetMessageId || m.SK === targetMessageId)
                  }
                  ...
                  convId={selectedChat?.id}
                  onVotePoll={async (messageId, optionIndex) => {
                    const { votePoll } = useChatStore.getState();
                    if (selectedChat?.id) {
                      await votePoll(selectedChat.id, messageId, optionIndex);
                    }
                  }}
                  onClosePoll={async (messageId) => {
                    const { closePoll } = useChatStore.getState();
                    if (selectedChat?.id) {
                      await closePoll(selectedChat.id, messageId);
                    }
                  }}
                />
              );
        ...
        <ChatModals
          ...
          onRecall={(m) =>
            patchMessageOptimistic(selectedChat.id, m.id, { action: "recall" })
          }
          onDelete={(m) => deleteMessageOptimistic(selectedChat.id, m.id)}
          onPin={handlePinMessage}
          userEmail={currentUserEmail || ""}
          showMuteMenuModal={showMuteMenuModal}
          setShowMuteMenuModal={setShowMuteMenuModal}
          onSelectMuteSchedule={handleSelectMuteSchedule}
          showCustomMuteModal={showCustomMuteModal}
          setShowCustomMuteModal={setShowCustomMuteModal}
          customMuteStartTime={customMuteStartTime}
          setCustomMuteStartTime={setCustomMuteStartTime}
          customMuteEndTime={customMuteEndTime}
          setCustomMuteEndTime={setCustomMuteEndTime}
          onApplyCustomMuteSchedule={async () => {
            if (
              !isValidTimeString(customMuteStartTime) ||
              !isValidTimeString(customMuteEndTime)
            )
              return Alert.alert("Lỗi", "Giờ không hợp lệ");
            const nextMap: Record<string, any> = {
              ...muteScheduleMap,
              [selectedChat.id]: createCustomWindowMuteSchedule(
                customMuteStartTime,
                customMuteEndTime,
              ),
            };
        ...
        <ForwardModal
          ...
          onForward={(targetId) =>
            sendMessageOptimistic(
              ...
              actionMessage.type,
              { attachments: actionMessage.media || actionMessage.files },
            )
          }
        />
      </View>
